...
void initVBOProgram0() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * banana_mesh.size(),
               banana_mesh.data(), GL_STATIC_DRAW);
  checkOpenGLerror();
}
...
void initVBOProgram1() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * spider_monkey_mesh.size(),
               spider_monkey_mesh.data(), GL_STATIC_DRAW);
  checkOpenGLerror();
}
...
void initShaderProgram0() {
  ...
  Attrib_vertex_texture_coordinate0 =
      glGetAttribLocation(Program0, attr_name_texture_coordinate0);
  if (Attrib_vertex_texture_coordinate0 == -1) {
    std::cout << "could not bind attrib " << attr_name_texture_coordinate0
              << std::endl;
    return;
  }
...
void initShaderProgram1() {
  ...
  Attrib_vertex_texture_coordinate1 =
      glGetAttribLocation(Program0, attr_name_texture_coordinate1);
  if (Attrib_vertex_texture_coordinate1 == -1) {
    std::cout << "could not bind attrib " << attr_name_texture_coordinate1
              << std::endl;
    return;
  }
...
void initTextureProgram0() {
  ...
  image_banana = SOIL_load_image(filename_banana, &image_width_banana,
                                 &image_height_banana, NULL, SOIL_LOAD_RGBA);
  glActiveTexture(GL_TEXTURE0);
  glGenTextures(1, &texture0);
  glBindTexture(GL_TEXTURE_2D, texture0);

  if (image_banana) {
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image_width_banana,
                 image_height_banana, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 image_banana);
...
void initTextureProgram1() {
  ...
  image_spider_monkey =
      SOIL_load_image(filename_spider_monkey, &image_width_spider_monkey,
                      &image_height_spider_monkey, NULL, SOIL_LOAD_RGBA);
  glActiveTexture(GL_TEXTURE1);
  glGenTextures(1, &texture1);
  glBindTexture(GL_TEXTURE_2D, texture1);

  if (image_spider_monkey) {
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image_width_spider_monkey,
                 image_height_spider_monkey, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 image_spider_monkey);
    glGenerateMipmap(GL_TEXTURE_2D);
  } else {
    std::cout << "could not load texture " << filename_spider_monkey
              << std::endl;
    return;
  }
}
...
void drawProgram0() {
  ...
  glVertexAttribPointer(Attrib_vertex_position0, 3, GL_FLOAT, GL_FALSE,
                        sizeof(Vertex), (GLvoid *)offsetof(Vertex, position));
  glVertexAttribPointer(Attrib_vertex_texture_coordinate0, 2, GL_FLOAT,
                        GL_FALSE, sizeof(Vertex),
                        (GLvoid *)offsetof(Vertex, texcoord));

  glVertexAttribPointer(Attrib_vertex_normal0, 3, GL_FLOAT, GL_FALSE,
  ...
  glUniformMatrix4fv(glGetUniformLocation(Program0, "model"), 1, GL_FALSE,
                     glm::value_ptr(model));
  glUniformMatrix4fv(glGetUniformLocation(Program0, "view"), 1, GL_FALSE,
                     glm::value_ptr(view));
  glUniformMatrix4fv(glGetUniformLocation(Program0, "projection"), 1, GL_FALSE,
                     glm::value_ptr(projection));

  glUniform1i(glGetUniformLocation(Program0, "texture"), 0);
  glDrawArrays(GL_TRIANGLES, 0, banana_mesh.size());
...
void drawProgram1() {
  ...
  glVertexAttribPointer(Attrib_vertex_position1, 3, GL_FLOAT, GL_FALSE,
                        sizeof(Vertex), (GLvoid *)offsetof(Vertex, position));
  glVertexAttribPointer(Attrib_vertex_texture_coordinate1, 2, GL_FLOAT,
                        GL_FALSE, sizeof(Vertex),
                        (GLvoid *)offsetof(Vertex, texcoord));
  ...
  glUniformMatrix4fv(glGetUniformLocation(Program1, "model"), 1, GL_FALSE,
                     glm::value_ptr(model));
  glUniformMatrix4fv(glGetUniformLocation(Program1, "view"), 1, GL_FALSE,
                     glm::value_ptr(view));
  glUniformMatrix4fv(glGetUniformLocation(Program1, "projection"), 1, GL_FALSE,
                     glm::value_ptr(projection));

  glUniform1i(glGetUniformLocation(Program1, "texture"), 1);
  glDrawArrays(GL_TRIANGLES, 0, spider_monkey_mesh.size());
...
int main() {
  sf::Window window(sf::VideoMode(windowWidth, windowHeight),
                    "Solar System Model", sf::Style::Default,
