...
auto camera = Camera(player, pitch, distanceToPlayer, worldUp, angleAroundPlayer);
...
void Draw()
{
    ...
    glUniform3f(glGetUniformLocation(Program, "spotLight.position"), spotLightPosition.x, spotLightPosition.y, spotLightPosition.z);
    glUniform3f(glGetUniformLocation(Program, "spotLight.direction"), spotLightDirection.x, spotLightDirection.y, spotLightDirection.z);
...
int main()
{
    sf::Window window(sf::VideoMode(windowWidth, windowHeight), "TankWorld", sf::Style::Default, sf::ContextSettings(24));
    ...
    while (window.isOpen())
    {
        ...
        framesRenderTimeDelta = static_cast<GLfloat>(fabs(currentFrameRenderTime - previousFrameRenderTime));
