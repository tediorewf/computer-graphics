...
public partial class MainForm : Form {
  ...
  private void DrawTriangulation(HashSet<Triangle2D> triangulation) {
    ...
    foreach (var edge in edges) {
      drawingSurface.DrawBresenhamLine(edge.Begin.ToPoint(), edge.End.ToPoint(),
                                       Color.Red);
    }
    triangulationPictureBox.Image = drawingSurface;
  }

  private void clearSceneButton_Click(object sender,
                                      EventArgs e) => ResetScene();
  ...
  private void ResetPoints() { points = new List<Point2D>(); }

  private void ResetTriangles() { triangulation = new HashSet<Triangle2D>(); }

  private void triangulationPictureBox_MouseClick(
      object sender, MouseEventArgs e) => DrawPoint(new Point2D(e.Location));

  private void DrawPoint(Point2D p) {
    var drawingSurface = triangulationPictureBox.Image as Bitmap;
    var gfx = Graphics.FromImage(drawingSurface);
    int radius = 5, diameter = radius * 2;
    gfx.FillEllipse(Brushes.Blue, p.X - radius, p.Y - radius, diameter,
                    diameter);
    triangulationPictureBox.Image = drawingSurface;
    points.Add(p);
  }
