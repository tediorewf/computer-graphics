...
public unsafe class FastBitmap : IDisposable {
  ...
  private byte *
  PixelOffset(Point point) => _scan0 // Указатель на начало потока байтов Bitmap
                              + point.Y * _stride // Сдвиг Y-той строки
                              + point.X
                                    * _bytesPerPixel; // Сдвиг X-того пикселя
  ...
  public Color GetPixel(Point point) {
    var data = PixelOffset(point);
    return Color.FromArgb(data[Channel.A], data[Channel.R], data[Channel.G],
                          data[Channel.B]);
  }
  ...
  /// </summary>
  public void Dispose() { _source.UnlockBits(_bitmapData); }
...
public static class FastBitmapTools {
  ...
  public static void ForEach(this Bitmap source, Action<Color> action) {
    using (var fastSource = new FastBitmap(source)) fastSource.ForEach(action);
  }
  ...
  /// <summary>
  /// Возвращает новую Bitmap, все пиксели которой получены преобразованием
  /// пикселей заданного Bitmap определённым образои.
  /// </summary>
  /// <param name="source">Искходны Bitmap.</param>
  /// <param name="transform">Преобразование, применяемое к пикселям исходного
  /// битмапа.</param>
  public static Bitmap Select(this Bitmap source,
                              Func<Color, Color> transform) {
    using (var fastSource =
               new FastBitmap(source)) return fastSource.Select(transform);
  }

  /// <summary>
  /// Возвращает новую Bitmap, все пиксели которой получены преобразованием
  /// пикселей заданного FastBitmap определённым образои.
  /// </summary>
  /// <param name="source">Искходны Bitmap.</param>
  /// <param name="transform">Преобразование, применяемое к пикселям исходного
  /// битмапа.</param>
  public static Bitmap Select(this FastBitmap source,
                              Func<Color, Color> transform) {
