...
public class Edge {
  ...
  public static Point Intersect(Point a, Point b, Point c, Point d) {
    ...
    if (!IsPointOnEdge(new Edge(a, b), p) ||
        !IsPointOnEdge(new Edge(c, d), p)) {
      throw new EdgesDontIntersectException();
    }
  ...
  private static bool IsPointOnEdge(Edge edge, Point p) {
    return Math.Min(edge.Begin.X, edge.End.X) <= p.X &&
           p.X <= Math.Max(edge.Begin.X, edge.End.X) &&
           Math.Min(edge.Begin.Y, edge.End.Y) <= p.Y &&
           p.Y <= Math.Max(edge.Begin.Y, edge.End.Y);
  }

  private static Point ParametrizePoint(Point begin, Point end, double t) {
    return new Point(begin.X + (int)(t * (end.X - begin.X)),
                     begin.Y + (int)(t * (end.Y - begin.Y)));
  }
