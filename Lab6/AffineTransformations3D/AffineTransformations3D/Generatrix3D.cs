...
public class Generatrix3D {
  ...
  private static Generatrix3D[] GenerateSkeletonBase(
      Generatrix3D beginGeneratrix, int partitionsCount) {
  ...
  private Generatrix3D Rotate(double degrees) {
    var rotated = Copy();
    TransformMultiplePointsInplace(rotated.Points,
                                   RotationAxis.CreateRotationMatrix(degrees));
    return rotated;
  }
  ...
  private static void MakeConnectionStep(Generatrix3D[] generatrices,
                                         List<Point3D> vertices,
                                         List<Edge3D> edges,
                                         List<Facet3D> facets, int iCurrent,
                                         int iNext) {
    var connectionEdges = new List<Edge3D>();
    for (int j = 0; j < generatrices[iCurrent].Points.Count; j++) {
      var connectionEdge = new Edge3D(generatrices[iCurrent].Points[j],
                                      generatrices[iNext].Points[j]);
    ...
    for (int k = 0; k < generatrices[iCurrent].Edges.Count; k++) {
      ...
      var currentFacetEdges =
          new List<Edge3D> { connectionEdge0, generatrixEdge0, generatrixEdge1,
                             connectionEdge1 };

      var currentFacetPoints = new List<Point3D> {
        connectionEdge0.Begin,
        connectionEdge0.End,
        connectionEdge1.Begin,
        connectionEdge1.End,
      };
