...
public class ColoredPointsGroupPair {
  ...
  public ColoredPointsGroupPair(List<DeptherizedPoint> left,
                                List<DeptherizedPoint> right) {
    Left = left;
    Right = right;
  }
}
...
public static class TriangleRasterisationAlgorithm {
  ...
  public static IEnumerable<DeptherizedPoint>
  RasteriseTriangle(DeptherizedPoint v1, DeptherizedPoint v2,
                    DeptherizedPoint v3) {
    ...
    foreach (var pointsGroupPair in pointsGroupPairs) {
      ...
      for (int x = leftSideMostRightPoint.X + 1; x < rightSideMostLeftPoint.X;
           x += 1) {
        var currentDepth = ApplyLinearInterpolation(
            leftSideMostRightPoint.Depth, rightSideMostLeftPoint.Depth, x,
  ...
  private static IEnumerable<List<DeptherizedPoint>>
  MakeBresenhamMovement(DeptherizedPoint v1, DeptherizedPoint v2) {
    int x1 = v1.X, x2 = v2.X;
    int y1 = v1.Y, y2 = v2.Y;
    ...
    while (x1 != x2 || y1 != y2) {
      var depthCurrent = ApplyLinearInterpolation(
          d1, d2, x1 * refereeY - y1 * refereeX, interpolationEndCoordinate,
  ...
  private static double ApplyLinearInterpolation(double beginDepth,
                                                 double endDepth,
    ...
    double depth =
        beginDepth * coordinateDeltaCurrent / coordinateDelta +
        endDepth * (coordinateDelta - coordinateDeltaCurrent) / coordinateDelta;
    return depth;
  }
}
}
