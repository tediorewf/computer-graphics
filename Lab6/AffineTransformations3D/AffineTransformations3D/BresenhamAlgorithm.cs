...
public static class BresenhamAlgorithm {
  ...
  public static void DrawBresenhamLineDashed(this Bitmap drawingSurface,
                                             Point p1, Point p2, Color color,
                                             int dashLength) {
    using (var fastDrawingSurface = new FastBitmap(drawingSurface))
        fastDrawingSurface.DrawPointsInterruptly(GetBresenhamPoints(p1, p2),
                                                 color, dashLength);
  }

  public static void DrawBresenhamLineDashed(this FastBitmap fastDrawingSurface,
                                             Point p1, Point p2, Color color,
                                             int dashLength) {
    fastDrawingSurface.DrawPointsInterruptly(GetBresenhamPoints(p1, p2), color,
                                             dashLength);
  }

  public static void DrawPoints(this FastBitmap fastDrawingSurface,
                                IEnumerable<Point> points, Color color) {
    foreach (var p in points) {
      fastDrawingSurface.SetPixelSafely(p, color);
    }
  }

  public static void DrawPointsInterruptly(this FastBitmap fastDrawingSurface,
                                           IEnumerable<Point> points,
                                           Color color,
                                           int interruptionLength) {
  ...
  private static void SetPixelSafely(this FastBitmap fastDrawingSurface,
                                     Point p, Color color) {
  ...
  private static bool IsPointOnSurface(this FastBitmap fastDrawingSurface,
                                       Point p) {
  ...
  private static void
  EnsureInterruptionLengthIsPositive(int interruptionLength) {
  ...
  public static IEnumerable<Point> GetBresenhamPoints(int x1, int y1, int x2,
                                                      int y2) {
    int diffX = x2 - x1;
    int diffY = y2 - y1;
