...
public class Polyhedron : ICloneable {
  ...
  public void RotateAroundCenter(double degreesX, double degreesY,
                                 double degreesZ) {
    /*var centeredRotationTransformation = MakeTranslationMatrix(-Center.X,
    -Center.Y, -Center.Z)
  ...
  public void RotateAroundPoint(double degreesX, double degreesY,
                                double degreesZ, Point3D p) {
  ...
  public object Clone() {
    ...
    var edges = new List<Edge3D>(Edges.Count); for (int i = 0; i < Edges.Count;
                                                    i++) {
      ...
      for (int j = 0; j < Vertices.Count; j++) {
        ...
        polyhedron.Color = Color; return polyhedron;
  }

  public Polyhedron Copy() => Clone() as Polyhedron;

  private Point3D ComputeCenter() => ComputeCenter(this);
