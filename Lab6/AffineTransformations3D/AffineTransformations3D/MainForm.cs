...
public partial class MainForm : Form {
  ...
  private void InitializeFacetsStuff() {
    facetsRemovingTypes = Enum.GetValues(typeof(FacetRemovingType))
                              .Cast<FacetRemovingType>()
                              .ToArray();
    facetsRemovingNames =
        facetsRemovingTypes.Select(frt => frt.GetFacetRemovingName()).ToArray();
    facetsRemovingComboBox.Items.AddRange(facetsRemovingNames);
        facetsRemovingComboBox.SelectedIndex = 0;
        currentFacetsRemovingType =
            facetsRemovingTypes[facetsRemovingComboBox.SelectedIndex];
  }
    ...
    foreach (var item in ListPolyhedron) {
      if (item != currentPolyhedron) {
        ...
        drawingSurface.DrawPolyhedron(
            camera.Project(removedFacets, currentProjectionType), Color.Blue);
      } else {
        var removedFacets = RemoveBackFacets(currentPolyhedron, viewPoint);
        drawingSurface.DrawPolyhedron(
            camera.Project(removedFacets, currentProjectionType), Color.Red);
      }
    }
  }
  ...
  private void DrawEdges(Bitmap drawingSurface) {
    foreach (var item in ListPolyhedron) {
      if (item != currentPolyhedron) {
        drawingSurface.DrawPolyhedron(
            camera.Project(item, currentProjectionType), Color.Blue);
      } else {
        drawingSurface.DrawPolyhedron(
            camera.Project(currentPolyhedron, currentProjectionType),
            Color.Red);
      }
    }
  }
  ...
  List<Facet3D> TriangulateFacet(Facet3D facet) {
    ...
    for (int i = 2; i < facet.Points.Count; i++) {
      var vertices = new List<Point3D> { firstPoint, facet.Points[i - 1],
                                         facet.Points[i] };
      var edges =
          new List<Edge3D> { new Edge3D(firstPoint, facet.Points[i - 1]),
  ...
  private void ZBufer(ZBuferStruct[,] ZBuferArr, Facet3D Triangle, Color Clr) {
    ...
    foreach (var item in LstPnt) {
      ...
      if ((item.X >= 0) && (item.X < size.Width) && (item.Y >= 0) &&
          (item.Y < size.Height)) {
        if ((!ZBuferArr[item.X, item.Y].IsNotEmpty) ||
            (depth > ZBuferArr[item.X, item.Y].Depth)) {
          var intensivity = item.Intensivity;
          var intensivityCOlor = Color.FromArgb((int)(Clr.R * intensivity),
                                                (int)(Clr.G * intensivity),
                                                (int)(Clr.B * intensivity));
  ...
  private void GorizontFunction(string FSTR, LimitationsFunction LF,
                                int splitting) {
    var size = polyhedronPictureBox.Size;
    var drawingSurface = new Bitmap(size.Width, size.Height);
  ...
  private void facetsRemovingComboBox_SelectedIndexChanged(object sender,
                                                           EventArgs e) {
    currentFacetsRemovingType =
        facetsRemovingTypes[facetsRemovingComboBox.SelectedIndex];
    Project();
  ...
  private void zBufferUnite(ZBuferStruct[,] ZBuferArr, ZBuferStruct[,] ZB) {
    ...
    for (int i = 0; i < size.Width; i++)
      for (int j = 0; j < size.Height; j++)
        if (ZB[i, j].IsNotEmpty &&
            ((ZBuferArr[i, j].IsNotEmpty &&
              (ZB[i, j].Depth > ZBuferArr[i, j].Depth)) ||
             !ZBuferArr[i, j].IsNotEmpty))
