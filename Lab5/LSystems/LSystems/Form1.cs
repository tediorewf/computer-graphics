...
public partial class Form1 : Form {
  ...
  private List<Edge> MakeEdgesTree(string mainstr, double corner,
                                   int iteration) {
  ...
  private void PaintEdgesTree(List<Edge> edges) {
    ...
    using (var fastDrawingSurface = new FastBitmap(drawingSurface)) {
      foreach (var edge in edges) {
        Color NewColor =
            Color.FromArgb(DrawingTreeBrown.R * edge.Proc_clr / pred,
                           (DrawingTreeBrown.G * edge.Proc_clr +
                            DrawingTreeGreen.G * (pred - edge.Proc_clr)) /
                               pred,
                           DrawingTreeBrown.B * edge.Proc_clr / pred);
