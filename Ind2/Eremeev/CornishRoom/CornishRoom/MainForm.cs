...
public partial class MainForm : Form {
  ...
  private void InitializeScene() {
    cornishRoomPictureBox.Image =
        new Bitmap(cornishRoomPictureBox.Width, cornishRoomPictureBox.Height);
    ...
    var lowerPlane = new Plane(new Vector3D(0.0, -2.0, 0.0),
                               new Vector3D(0.0, 1.0, 0.0), pinkMaterial);
    var rightPlane = new Plane(new Vector3D(8.0, 0.0, 0.0),
                               new Vector3D(-1.0, 0.0, 0.0), orangeMaterial);
    var leftPlane = new Plane(new Vector3D(-8.0, 0.0, 0.0),
                              new Vector3D(1.0, 0.0, 0.0), blueMaterial);
    var backPlane =
        new Plane(new Vector3D(0.0, 0.0, 30.0), new Vector3D(0.0, 0.0, -1.0),
                  yellowGreenMaterial);
    var upperPlane = new Plane(new Vector3D(0.0, 10.0, 0.0),
                               new Vector3D(0.0, -1.0, 0.0), cyanMaterial);
    var frontPlane =
        new Plane(new Vector3D(0.0, 0.0, -30.0), new Vector3D(0.0, 0.0, 1.0),
                  yellowGreenMaterial);
    ...
    lights = new List<Light> { new Light(new Vector3D(2.0, 1.0, 0.0), 0.3),
  ...
  private void renderButton_Click(object sender, EventArgs e) {
    ...
    var renderer = new Renderer(pointOfView, cornishRoomPictureBox.Size,
                                primitives, lights, backgroundColor);
